...
const {
  editor: { buildFromText },
} = Helpers
const {
  postAbstract: { DEFAULT_ATOM_NAME },
} = Helpers

let editor: Editor
let editorElement: HTMLElement

function renderEditor(treeFn: BuildCallback, editorOptions: EditorOptions = {}) {
  editor = Helpers.mobiledoc.renderInto(editorElement, treeFn, editorOptions)
...
test('typing shift-ENTER produces a soft return', assert => {
  editor = Helpers.editor.buildFromText('abc|', { element: editorElement })

  Helpers.dom.triggerKeyCommand(editor, 'ENTER', [MODIFIERS.SHIFT])

  assert.hasElement('#editor .-mobiledoc-kit__atom br', 'has br')
})
