...
test('stateDidChange callback fired when markup is toggled and there is a selection', (assert) => {
  ...
  let stateChanged = 0;
  editor.stateDidChange(() => stateChanged++);

  editor.toggleMarkup('b');
  setTimeout(() => {
    assert.equal(stateChanged, 1, 'stateDidChange fired once');
    done();
  });
});

test('stateDidChange callback fired when markup is toggled and there is no selection', (assert) => {
  let done = assert.async();
  assert.expect(1);
...
test('stateDidChange callback fired when moving cursor into markup', (assert) => {
  let done = assert.async();
  assert.expect(1);
...
test('stateDidChange callback fired when toggling section', (assert) => {
  let done = assert.async();
  assert.expect(1);
...
test('stateDidChange callback not fired when toggle is no-op', (assert) => {
  let done = assert.async();
  assert.expect(1);
  ...
  setTimeout(() => {
    assert.equal(stateChanged, 0, 'stateDidChange not fired');
    done();
  });
});

test('stateDidChange callback fired when moving cursor into section', (assert) => {
  let done = assert.async();
  assert.expect(2);
...
test('stateDidChange callback not fired when moving cursor into same section', (assert) => {
  let done = assert.async();
  assert.expect(2);
