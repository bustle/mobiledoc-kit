...
test('leaves modifier, code and run in place if they exist', assert => {
  ...
  const { modifier, code, run } = buildKeyCommand({
    code: Keycodes.ENTER,
...
test('translates MODIFIER+CHARACTER string to modifierMask and code', assert => {
  ...
  assert.equal(modifierMask, createModifierMask({ metaKey: true }), 'calculates correct modifierMask')
...
test('translates multiple modifiers to modifierMask', assert => {
  ...
  assert.equal(modifierMask, createModifierMask({ metaKey: true, shiftKey: true }), 'calculates correct modifierMask')
  assert.equal(code, 75, 'translates string to code')
})
